...
const QuestionForm = ({ question, isEdit }: IParams) => {
  ...
  const form = useForm<z.infer<typeof AskQuestionSchema>>({
    ...
    defaultValues: {
      title: question?.title || EMPTY_CHAR,
      content: question?.content || EMPTY_CHAR,
  ...
  const handleInputKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, field: { value: string[] }) => {
    if (e.key === "Enter") {
      ...
      if (tagInput && tagInput.length < 15 && !field.value.includes(tagInput)) {
        ...
        form.setError("tags", { type: "manual", message: "Tag should be less than 15 " });
      } else if (field.value.includes(tagInput)) {
        form.setError("tags", { type: "manual", message: "Tag alredy exists " });
      }
    }
  };
