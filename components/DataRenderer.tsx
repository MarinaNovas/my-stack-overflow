...
import { DEFAULT_EMPTY, DEFAULT_ERROR } from "@/constans/state";
...
const StateSkeleton = ({ image, title, message, button }: IStateSkeletonProps) => (
  <div className="mt-10 flex w-full flex-col items-center justify-center sm:mt-36">
    <>
...
const DataRenderer = <T,>({ success, error, data, empty = DEFAULT_EMPTY, render }: IProps<T>) => {
  if (!success) {
    return (
      <StateSkeleton
        image={{
